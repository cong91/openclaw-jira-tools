...
export function registerInspectTool(api: OpenClawPluginApi, rawConfig: JiraToolsConfig) {
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: any) {
      try {
        ...
        if (issueType) createmeta = await client.getCreateMeta(projectKey, issueType);
        return createToolResult(JSON.stringify({
          key: project.key,
          name: project.name,
          issueTypes: (project.issueTypes || []).map((it: any) => ({ id: it.id, name: it.name, subtask: it.subtask })),
