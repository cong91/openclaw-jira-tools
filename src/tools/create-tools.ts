...
import type { JiraCreateInput, JiraToolsConfig } from "../shared/types.js";
...
export function registerCreateTools(api: OpenClawPluginApi, rawConfig: JiraToolsConfig) {
  ...
  async function loadDefaults(ctx: any) {
  ...
  async function createStandardIssue(kind: "task" | "epic" | "subtask", input: JiraCreateInput & { epic?: string; parent?: string; parentLink?: string }, ctx: any) {
    ...
    if (kind === "epic") fields[client["config"].fieldMappings.epicName] = merged.summary;
    if (kind === "task" && input.epic) fields[client["config"].fieldMappings.epicLink] = input.epic;
    if (kind === "task" && input.parentLink) fields[client["config"].fieldMappings.parentLink] = input.parentLink;
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: any, ctx: any) {
      try { return createToolResult(await createStandardIssue("task", params, ctx)); }
      catch (e) { return createToolResult(`Error: ${e instanceof Error ? e.message : String(e)}`, true); }
    }
  }, { optional: true });
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: any, ctx: any) {
      try { return createToolResult(await createStandardIssue("epic", params, ctx)); }
      catch (e) { return createToolResult(`Error: ${e instanceof Error ? e.message : String(e)}`, true); }
    }
  }, { optional: true });
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: any, ctx: any) {
      try { return createToolResult(await createStandardIssue("subtask", params, ctx)); }
      catch (e) { return createToolResult(`Error: ${e instanceof Error ? e.message : String(e)}`, true); }
    }
  }, { optional: true });
}
