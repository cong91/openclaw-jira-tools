...
export function registerManagementTools(api: OpenClawPluginApi, rawConfig: JiraToolsConfig) {
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: any) {
      try {
        ...
        if (params.description) fields.description = client.buildDescription("Update", params.summary || params.issueKey, params.description, params.description, params.description, []);
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: any) {
      try {
        ...
        if (params.internal) payload.properties = [{ key: "sd.public.comment", value: { internal: true } }];
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: any) {
      try {
        ...
        if (assigneeAccountId) cloneFields.assignee = { accountId: assigneeAccountId };
        if (params.parent) cloneFields.parent = { key: params.parent };
        if (params.priority) cloneFields.priority = { name: params.priority };
  ...
  api.registerTool({
    ...
    async execute(_id: string, params: any) {
      try {
        const payload: any = { timeSpent: params.timeSpent };
        if (params.started) payload.started = params.started;
        if (params.comment) {
          payload.comment = {
            type: "doc",
            version: 1,
        ...
        if (params.newEstimate) payload.adjustEstimate = "new";
        if (params.newEstimate) payload.newEstimate = params.newEstimate;
